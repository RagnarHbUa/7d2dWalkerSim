...
namespace WalkerSim
{
    ...
    public class Simulation
    {
        ...
        static string SimulationFile = string.Format("{0}/WalkerSim.bin", GameIO.GetSaveGameDir());
		...
		int respawn_day = 0;
		int respawn_day_frequency_increasing_coefficient = 4;
		int respawn_day_frequency = GamePrefs.GetInt(EnumGamePrefs.BloodMoonFrequency) * 6;
        ...
        public void Start()
        {
            ...
            {
				Log.Out("[WalkerSim] Performing reset on start because of not persistent mode or failed load.");
                Reset();
            }
        ...
        public void Save()
        {
			...
			Log.Out("[WalkerSim] Going to save simulation.");
        ...
        public bool Load()
        {
            ...
            {
				Log.Out("[WalkerSim] Exception on loading stored simulation data: {0} --- {1} --- {2}", e.Message, e.Source, e.StackTrace);
				Log.Exception(e);
        ...
        public void Reset()
        {
            ...
            // Populate
            CreateInactiveRoaming(true);

            _nextSave = DateTime.Now.AddMinutes(5);
        }

        void CreateInactiveRoaming(bool initial)
        {
            int numCreated = 0;
        ...
        {
            POIZone closest = null;
			float distance_to_search_poi = 100;
			float increment_of_distance_to_search_poi = 100;
			
			while (closest == null) {
				closest = _pois.GetRandomClosest(zombie.pos, _prng, distance_to_search_poi, zombie.visitedZones);
				
				if (closest == null) {
					distance_to_search_poi += increment_of_distance_to_search_poi;
					
					if (distance_to_search_poi > 9000) {
		...
		private void respawnDeadInactiveByTimer(ulong world_time)
		{			
			int current_day = GameUtils.WorldTimeToDays(world_time);
			
			while(current_day > respawn_day)
			{
				respawn_day = respawn_day + respawn_day_frequency;
			}
			
			if (current_day == respawn_day) {
				Log.Out("[WalkerSim] Respawn day {0} is now!", respawn_day);
				CreateInactiveRoaming(false);
				
				respawn_day = respawn_day + respawn_day_frequency;
				Log.Out("[WalkerSim] Next respawn day is {0}!", respawn_day);
			}		
		}
        ...
        private void UpdateInactiveZombies(float dt)
        {
			...
			respawnDeadInactiveByTimer(world.GetWorldTime());
